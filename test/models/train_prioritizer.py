...
from test.models.models import ExpansionModel
...
class transform(object):
    ...
    def __call__(self,sample):
        ...
        fp_p = self.fp(p,self.nBits)        
        label = l
        return {'fp' : torch.Tensor(fp_p), 'label' : label}
    ...
    def fp(self,smi,nBits):
        ...
        return arr
#class transform(object):
#    def __init__(self, nBits=2048,radius=2):
#        self.nBits=nBits
#        self.radius=radius
#    def __call__(self,sample):
#        """reactants
...
#        r,p,l = sample['reactants'], sample['product'], sample['template_idx']
#        
#        fp_p = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(p),
#                                                     radius=self.radius,
#                                                     nBits=self.nBits)
#        label = l
#        return {'fp':torch.Tensor(fp_p),'label':label}
...
class MyDataset(Dataset):
    def __init__(self, file_name,radius=2,nBits=1024):
        with open(file_name,'rb') as f:
            self.data = pickle.load(f)
        self.data = self.data[:int(len(self.data)*0.25)]
...
def read_config(config_file):
    ...
    keys = ['train_option_prioritizer', 'expansion_model']
    name = 'expansion'+'_'.join([str(v) for k,v in config['expansion_model'].items()])

    config_ = config['train_option_prioritizer']
    default = {}
    ...
    return default, config['expansion_model']
...
num_templates = model_config['num_templates']
...
tr_dataloader = DataLoader(tr_ds, batch_size=batch_size, shuffle=True, num_workers=num_workers)
val_dataloader = DataLoader(val_ds, batch_size=batch_size, shuffle=True, num_workers=num_workers)
...
scheduling_step = [10, 50,]
scheduler = torch.optim.lr_scheduler.MultiStepLR(
                        optimizer,
                        milestones= [(i+1)*len(tr_dataloader) for i in 
                                                            scheduling_step],
                        gamma=0.25)
if train_from:
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(tr_dataloader):
        fp,label = data['fp'],data['label']
        fp = fp.to(device); label = label.to(device)
        optimizer.zero_grad()
