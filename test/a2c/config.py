...
from test.utils.paths import data_path
from test.context.policy import TemplatePolicy, PairPolicy
from test.context.stock import Stock
from test.context.scoring import ScorerCollection

if TYPE_CHECKING:
    from test.utils.type_utils import StrDict, Any
...
class Configuration:
    ...
    template_policy: TemplatePolicy = None  # type: ignore
    pair_policy: PairPolicy = None  # type: ignore
    ...
    def __post_init__(self) -> None:
        ...
        self.stock = Stock()
        self.template_policy = TemplatePolicy(self)
        self.pair_policy = PairPolicy(self)
    ...
    def from_dict(cls, source: StrDict) -> "Configuration":
        ...
        config_obj.template_policy.load_from_config(
            **source.get("policy", {}).get("files", {})
        )
        config_obj.pair_policy.load_from_config(
            **source.get("pair", {}).get("files", {})
        )
